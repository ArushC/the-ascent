...
export const INSERT_SCORE_RUN_SQL = `
  INSERT INTO score_runs (player_id, score, mode, challenge_date, created_at)
  VALUES (?, ?, ?, ?, ?)
`;
...
export const SELECT_TOP_SCORE_RUNS_FOR_PLAYER_SQL = `
  ...
  FROM score_runs
  WHERE player_id = ? AND mode = 'classic'
  ORDER BY score DESC, created_at ASC
  LIMIT ?
`;

export const SELECT_TOP_DAILY_SCORE_RUNS_FOR_PLAYER_SQL = `
  SELECT score, created_at
  FROM score_runs
  WHERE player_id = ? AND mode = 'daily' AND challenge_date = ?
  ORDER BY score DESC, created_at ASC
  LIMIT ?
`;
...
export const DELETE_EXCESS_PLAYER_SCORE_RUNS_SQL = `
  ...
  WHERE player_id = ? AND mode = ? AND challenge_date IS ?
    AND id NOT IN (
      SELECT id
      FROM score_runs
      WHERE player_id = ? AND mode = ? AND challenge_date IS ?
...
export const SELECT_BEST_DAILY_SCORE_SQL = `
  SELECT MAX(score) AS best_score
  FROM score_runs
  WHERE player_id = ? AND mode = 'daily' AND challenge_date = ?
`;

export const INSERT_PLAYER_IF_MISSING_SQL = `
  INSERT INTO players (player_id, updated_at)
  VALUES (?, ?)
