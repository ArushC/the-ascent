...
export async function runSpec(state: RunState): Promise<string> {
  ...
  const prompt = appendRevisionFeedback(`${renderPrompt("spec", params)}\n\nInspect the repository before answering. Scope milestones only to this requested change. Do not rebuild, redeploy, or reimplement existing systems. Return Markdown only.`, state);
